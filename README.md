...
- 이번 무중단 배포에서의 핵심은 2가지로 생각합니다. intelij에서 깃허브로 push를 했을 때 travis에 연동되어있는 Repository가 자동으로 빌드를 하여 검사를 하는 포인트와 빌드를 다 진행하고 *.jar파일을 s3와 CodeDepoly에서 받고 **쉡 스크립트**를 이용하여 배포하는 과정의 포인트 입니다.
